...
Polite: ~5 req/sec, 429 backoff, fully resumable.

Usage:
    python backfill_historical.py --start 2024-Q1 --end 2024-Q1 --dry-run
    python backfill_historical.py --start 2024-Q1 --end 2024-Q4
    python backfill_historical.py --years 4
    python backfill_historical.py --years 4 --resume
    python backfill_historical.py --years 4 --workers 2   # if getting 429s
─────────────────────────────────────────────────────────────────────────────
"""
...
# ── Config — these are module-level so main() can override them ────────────────
...
def sec_get(url: str, params: dict = None, timeout: int = 25,
    ...
    for attempt in range(max_retries):
        ...
        if r.status_code == 429:
            wait = 65*(attempt+1)
            log.warning(f"429 — waiting {wait}s (attempt {attempt+1})"); time.sleep(wait); continue
...
# ── Step 1: Parse form.idx for the quarter ─────────────────────────────────────
...
def fetch_quarter_ciks(year: int, quarter: int) -> dict[str, list[dict]]:
    ...
    for line in lines[start:]:
        ...
        if form_type not in ("4","4/A"): continue
        try:
...
def get_docs_for_cik(cik_str: str) -> dict[str, str]:
    """Returns {accession_nodash: primary_xml_filename} for all Form 4s."""
...
def process_cik(cik_str: str, filings: list[dict]) -> list[Transaction]:
    ...
    for meta in filings:
        nodash    = meta["nodash"]
        cik_pad   = meta["cik_padded"]
        accession = meta["accession"]
        base_path = f"{EDGAR_BASE}/Archives/edgar/data/{cik_pad}/{nodash}"

        doc_name  = doc_map.get(nodash)

        if not doc_name:
            url = guess_xml_url(cik_pad, nodash)
        else:
            # Handle old SGML .txt filings — fetch index page to find embedded XML
            if not doc_name.lower().endswith(".xml"):
        ...
        if xml_resp is None: continue

        txns = parse_form4_xml(xml_resp.text, accession, cik_str,
...
def upsert(conn, txns: list[Transaction]) -> int:
    rows = [t.to_tuple() for t in txns]
    with conn:
        conn.cursor().executemany(UPSERT_SQL, rows)
    return len(rows)

def get_existing(conn) -> set[str]:
...
def process_quarter(year: int, quarter: int, existing: set[str], conn) -> tuple[int,int]:
    ...
    all_txns=[]; done=0; nciks=len(by_cik)
    ...
    if DRY_RUN:
        log.info(f"  DRY RUN sample:\n{json.dumps(asdict(all_txns[0]), default=str, indent=2)}")
        return total, 0

    written = 0
    for i in range(0, len(all_txns), 500):
        written += upsert(conn, all_txns[i:i+500])
...
def main():
    # Declare globals FIRST before any other code in this function
    ...
    ap.add_argument("--resume",  action="store_true",
                    help="Skip accessions already in DB")
    ap.add_argument("--workers", type=int,
                    help="Parallel workers (default 3, max 4)")
    args = ap.parse_args()

    if args.dry_run: DRY_RUN = True
    if args.workers: MAX_WORKERS = min(args.workers, 4)

    today = date.today(); cy, cq = quarter_of(today)
    ...
    if args.resume and conn:
        log.info("Loading existing accessions…")
    ...
    for i,(y,q) in enumerate(qs,1):
        log.info(f"\n[{i}/{len(qs)}]  {y}-Q{q}")
        try:
            f,w = process_quarter(y,q,existing,conn)
            tf+=f; tw+=w
        except KeyboardInterrupt:
            log.info("Interrupted — re-run with --resume to continue"); break
        except Exception as e:
